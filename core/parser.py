...
def safe_parse_date(date_str):
    ...
    try:
        ...
        try:
            return datetime.datetime.fromisoformat(date_str).astimezone(datetime.timezone.utc).isoformat()
        except Exception:
            return None
    return None
...
# parse CSS style inline for width/height/display/opacity
...
def is_tracking_image_tag(tag):
    """
    Heuristics for tracking pixel detection for a BeautifulSoup <img> tag:
    - width/height attributes <= 1
    - inline style contains width/height 1px or display:none or opacity:0
    - src is remote (not data:) and contains typical tracker substrings (awstrack, track, tracker) OR size heuristics
    """
    if not tag:
        return False
    src = (tag.get("src") or "").strip()
    # embedded data URIs aren't remote tracking (but could be embedded images)
    is_data = src.startswith("data:")
    # check numeric width/height attributes
    try:
    ...
    for k in ("width", "height"):
        val = styles.get(k)
        if val:
            # accept '1px', '1', '1em' heuristic - check for '1'
            if re.search(r'\b1(px)?\b', val):
                return True
    # invisible styling
    if styles.get("display") == "none" or styles.get("opacity") in ("0", "0.0"):
        return True
    # src heuristics for common tracking tokens (awstrack, track, pixel)
...
def detect_tracking_images(html):
    """
    Returns a dict:
    - tracking_pixel_detected (bool)
    - remote_image_urls (list)
    - embedded_base64_images_count (int)
    """
    if not html:
...
# ---------------------------
# Core parse for a single message
# ---------------------------
def parse_eml_bytes(raw_bytes, tempdir=None, message_index=None):
    ...
    # body extraction with charset + cte
    body_text = None
    body_html = None
    body_charset = None
    body_cte = None  # content-transfer-encoding of the body part we used
    try:
        if msg.is_multipart():
            # prefer text/plain for text, text/html for html; capture charset and cte from the parts we consume
            for part in msg.walk():
                ctype = part.get_content_type()
                cdisp = part.get_content_disposition()  # 'attachment' or 'inline' or None
                if cdisp == "attachment":
                    continue
                try:
                    payload = part.get_payload(decode=True)
                ...
                if not payload:
                    continue
                # charset detection (prefer part charset)
    ...
    # IOC extraction from combined text
    combined_text = (body_text or "") + "\n" + (body_html or "")
    iocs = extract_iocs(combined_text)

    # html tracking detection (improved)
    tracking = detect_tracking_images(body_html)

    # obfuscation detection heuristics (kept simple)
    obf = {
    ...
    for part in msg.walk():
        # consider attachment if content-disposition=attachment OR filename present
    ...
    # quick display-name spoof detection
    try:
...
# File-level parse
# ---------------------------
def parse_input_file(path, tempdir=None):
    p = Path(path)
    ...
    if suffix == ".mbox" or suffix == ".mbx":
        try:
            ...
            for msg in mbox:
                ...
                parsed = parse_eml_bytes(raw_bytes, tempdir=tempdir, message_index=idx)
                parsed["meta"] = {"message_index": idx}
                results.append(parsed)
            metadata["isMultipart"] = None
        ...
        try:
            ...
            if extract_msg:
                ...
                parsed = parse_eml_bytes(pseudo_eml, tempdir=tempdir, message_index=1)
                results.append(parsed)
            else:
        ...
        try:
            ...
            parsed = parse_eml_bytes(raw_bytes, tempdir=tempdir, message_index=1)
            results.append(parsed)
...
# ---------------------------
def main():
    parser = argparse.ArgumentParser(description="Email Forensics Extractor - Patch v2")
    parser.add_argument("input_file", help="Path to .eml/.txt/.mbox/.msg file")
    parser.add_argument("--output", "-o", help="Output JSON file (default: <input>_forensics.json)")
    parser.add_argument("--keep-temp", action="store_true", help="Keep temporary extraction files")
    ...
    outpath = Path(args.output) if args.output else Path(inpath.stem + "_forensics_v2.json")
    tempdir = tempfile.mkdtemp(prefix="email_forensics_patch2_")
    try:
        metadata, messages = parse_input_file(str(inpath), tempdir=tempdir)
        out = {
            "meta": metadata,
            "messages": messages,
            "extractedAt": now_iso(),
            "tool": {"name": "EmailForensicsPatch", "version": "2.0"},
        }
        with open(outpath, "w", encoding="utf-8") as f:
            json.dump(out, f, indent=2, ensure_ascii=False)
        logging.info(f"Exported forensic JSON → {outpath}")
        if args.keep_temp:
            logging.info(f"Temporary files kept at {tempdir}")
        else:
